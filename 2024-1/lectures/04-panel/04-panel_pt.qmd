...
reg_dummy = smf.ols(formula="crmrte ~ -1 + prbarr + C(county)", data=css).fit()

pd.concat(
    [reg_dummy.params, reg_dummy.bse, reg_dummy.tvalues, reg_dummy.pvalues], axis=1
...
# Get predicted line
css["pred"] = reg_dummy.predict(css)
...
- `r emo::ji("warning")` Isso não é verdade!!
...
&=E[X_{it}u_{it}]+E[X_{it}\bar{u}_i]+\ldots
\end{align*}
$$
...
Ou seja, a hipótese de exogeneidade estrita, $E[u_{it}\mid X_{i1},\ldots, X_{iT}, c_i]=0$ implica em $E[X_{is}u_{it}]=0\quad s,t = 1,\ldots, T$, garante que no modelo transformado o MQO é não viesado 
...
- **Python**
  ...
  + [`linearmodels`](https://pypi.org/project/linearmodels/) (EF em duas vias)
...
| <a href="https://raw.githack.com/rfbressan/econometria3_slides/master/lectures/06-panel/lista_IV_pt.html">.ScPored[<i class="fa fa-list fa-fw"></i>] | Lista de Exercícios IV |
