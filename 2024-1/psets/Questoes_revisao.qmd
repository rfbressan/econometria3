...
$Y_i=\beta X_i + u_i$, $i=1,2,\ldots,n$ e o termo de erro $u_i$ é tal que $E[u_i]=0$ e $\text{Var}(u_i)=\sigma^2$.
...
Também considere que as hipóteses de Gauss-Markov são válidas, ou seja, $E[u|X]=0$ e $\text{Var}(u)=\sigma^2I_n$.
...
d. Mostre que $\hat{Y} = PY$ e $\hat{u} = MY$.

e. Mostre que $\hat{Y}$ e $\hat{u}$ são ortogonais, ou seja, $\hat{Y}'\hat{u} = 0$.
...
a. Faça uma regressão de $Y$ em $X_1$ e obtenha os resíduos chamando-os de $\tilde{Y} = M_1Y$.

b. Faça uma regressão de $X_2$ em $X_1$ e obtenha os resíduos chamando-os de $\tilde{X}_2 = M_1X_2$.

c. A partir da equação de regressão particionada, mostre que $\tilde{Y}=\tilde{X}_2\beta_2+\tilde{u}$.

d. Mostre que o estimador de MQO de $\beta_2$ na equação acima é dado por $\hat{\beta}_2 = (\tilde{X}_2'\tilde{X}_2)^{-1}\tilde{X}_2'\tilde{Y}$.
