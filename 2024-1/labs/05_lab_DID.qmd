...
Nesta aula vamos fazer uso do `R`, além de `Python`. Não encontrei nenhuma maneira razoavelmente simples de fazer **estudos de eventos** em Python, portanto, para esta parte da aula iremos recorrer ao `R`. 
...
Modelos DID são atualmente os mais utilizados em estudos observacionais para investigar os efeitos de alguma política em uma variável de interesse. Este tipo de _design_ faz uso das variações tanto entre grupo de indivíduos quanto no tempo para obter o **efeito médio do tratamento sobre os tratados**
- ATT.
...
Em um formato de regressão, o estimador DID é obtido regredindo a variável de interesse em uma _dummy_ para o grupo tratamento, uma outra _dummy_ para o período após a intervenção e um termo de interação entre os dois, que denota o efetivo tratamento.
...
delta_t = (
    tratados[tratados["year"] == 1981]["rprice"].mean()
    - tratados[tratados["year"] == 1978]["rprice"].mean()
)
delta_c = (
    controles[controles["year"] == 1981]["rprice"].mean()
    - controles[controles["year"] == 1978]["rprice"].mean()
)
...
```{python}
did_reg = smf.ols("rprice ~ nearinc * y81", data=kielmc).fit(cov_type="HC1")
did_reg.summary()
```

Digamos que a hipótese de tendências paralelas seja válida somente após controlar pela idade do imóvel e idade ao quadrado, distância para uma rodovia, área do terreno e da moradia, número de quartos e banheiros. 

Gostaríamos também que o modelo fosse especificado em logarítimos, para ter uma ideia de variação percentual do preço da moradia em relação a presença do incinerador.
...
#| code-fold: true
did_ctr = smf.ols(
    "np.log(rprice) ~ nearinc * y81 + age + agesq + lintst + np.log(land) + np.log(area) + rooms + baths",
    data=kielmc
).fit(cov_type="HC1")
did_ctr.summary()
```

# Efeito dos eventos climáticos costeiros nos preços das casas

Especificamente, analisaremos o efeito da temporada de furacões no Atlântico de 2005, que foi a temporada de furacões mais ativa registrada na história até 2020. Construiremos uma regressão simples de Diferença em Diferenças para estudar o efeito da temporada de furacões de 2005 na mudança no Índice de Preços de Imóveis, também conhecido como inflação de preços de imóveis nos estados costeiros. 

Na nossa pequena experiência, o “tratamento” significará ser submetido a todo o impacto da temporada de furacões de 2005. A ‘unidade’ submetida (ou não) ao tratamento é um estado dos EUA com costa marítima. Existem 24 destes estados.

Decidiremos se um estado se enquadra no grupo de tratamento examinando as ações tomadas pela Agência Federal de Gestão de Emergências dos EUA (FEMA) naquele estado durante a temporada de furacões no Atlântico de 2005. Tentaremos resolver esta questão traçando a reta na mediana das contagens de condados que obtiveram auxílio. Qualquer estado com contagem maior ou igual à mediana (14) cairá no grupo de tratamento. O restante fará parte do grupo controle. Esta é a aparência do mapa do grupo:

![](https://miro.medium.com/v2/resize:fit:828/format:webp/1*JU3G6EKmVLXWDIVdWf-SiQ.png)

Vamos carregar os dados:

```{python}
df = pd.read_csv("data/us_fred_coastal_us_states.csv", header=0)
df.head()
```

Nossa variável dependente será `HPI_CHG`, que é a variação percentual no índice de preços de imóveis. Nossos regressores serão as _dummies_ `Time_Period` e `Disaster_Affected`, onde a primeira reflete o período de intempéries e a última indica se o estado foi afetado por desastres naturais em 2005 acima da mediana de condados que obtiveram auxílio.

**Especifique e estime a regressão DID**
```{python}
# | code-fold: true
did_reg = smf.ols("HPI_CHG ~ Time_Period * Disaster_Affected", data=df).fit(
    cov_type="HC1"
)
did_reg.summary()
```

O efeito do tratamento é **positivo** e estatisticamente significativo (i.e. 1,97 p.p). Isso significa que a inflação dos preços das casas foi maior nos estados costeiros que foram afetados por desastres naturais em 2005 em relação aos estados também costeiros mas que não foram tão fortemente impactados.

- Qual a racionalização econômica que você daria para este resultado?
...
Este tipo de design é o que chama-se de adoção escalonada ao tratamento (_staggered adoption_). Existem diversas coortes que aderem ao tratamento em momentos distintos
...
point-range, onde a estimativa pontual e o intervalo de confiança são mostrados. Vamos fazer isso agora.
...
Vejam que as estimativas mudaram ligeiramente apenas. Muito embora a especificação TWFE deva ser viesada em teoria, este viés aparentemente é pequeno. 